...
import { LucideIcon, TrendingUp, TrendingDown, MoveHorizontalIcon, CircleSlash } from 'lucide-react';
import { defaultCurrency, gridColsClass } from '@/functions/commonLists';
...
const PolicyMetricCards: React.FC<MetricCardsProps> = ({ metrics, namespace = 'common' }) => {
  const getDescription = (percent_change: number | null) => {
    ...
    return (
      <p className={cn(
        ...
        {Icon}
        {percent_change !== null ? `${percent_change > 0 ? '+' : ''}${percent_change}% ${t(`${namespace}:from_last_month`)}` : t(`${namespace}:not_enough_data_yet`)}
      </p>
    );
  };
